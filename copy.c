        ...
        for (u = t->use; u < &t->use[t->nuse]; u++) {
                ...
                case UIns:
                        ...
                        if (i->op == Oand) {
                                ...
                                if (isconbits(fn, rc, &v) && bitwidth(v) <= w) {
                                        continue;
                                }
                                break;
                        }
...
dwl(Fn *fn, Ref r, int w) {
        ...
        if (isconbits(fn, r, &v) && bitwidth(v) <= w) {
                return 1;
        }
        if (w <= 0) {
                return 0;
        ...
        if (i->op == Oand) {
                if (dwl(fn, i->arg[0], w) || dwl(fn, i->arg[1], w)) {
                        return 1;
                }
                return 0;
        }
...
Ref copyref(Fn *fn, Blk *b, Ins *i) {
        ...
        if (optab[i->op].hasid && KBASE(i->cls) == 0 /* integer only - fp NaN! */
            && req(i->arg[1], con01[optab[i->op].idval]) && (!optab[i->op].cmpeqwl || isw1(fn, i->arg[0]))) {
                return i->arg[0];
        }

        /* idempotent op with identical args */
        if (optab[i->op].idemp && req(i->arg[0], i->arg[1])) {
                return i->arg[0];
        }

        /* integer cmp with identical args */
        if ((optab[i->op].cmpeqwl || optab[i->op].cmplgtewl) && req(i->arg[0], i->arg[1])) {
                return con01[optab[i->op].eqval];
        }

        /* cmpeq/ne 0 with 0/non-0 inference */
        if (optab[i->op].cmpeqwl && req(i->arg[1], CON_Z) && zeroval(fn, b, i->arg[0], argcls(i, 0), &z)) {
                return con01[optab[i->op].eqval ^ z ^ 1];
        }

        /* redundant and mask */
        if (i->op == Oand && isconbits(fn, i->arg[1], &v) && (v > 0 && ((v + 1) & v) == 0) && defwidthle(fn, i->arg[0], bitwidth(v))) {
                return i->arg[0];
        }

        if (i->cls == Kw && (i->op == Oextsw || i->op == Oextuw)) {
                return i->arg[0];
        }
...
Ref phicopyref(Fn *fn, Blk *b, Phi *p) {
         ...
         * dominating jnz arg */
        d = b->idom;
        if (p->narg != 2 || d->jmp.type != Jjnz || !isw1(fn, d->jmp.arg)) {
                return R;
        }
        ...
        if (d->s1 == s[1] && d->s2 == s[0] && d->s1->jmp.type == Jjmp && d->s2->jmp.type == Jjmp) {
                return d->jmp.arg;
        }
