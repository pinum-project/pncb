...
int ifgraph(Blk *ifb, Blk **pthenb, Blk **pelseb, Blk **pjoinb) {
        Blk *s1, *s2, **t;
...
static int
jmpeq(Jmp *a, Jmp *b) {
        return a->type == b->type && req(a->arg, b->arg) && a->s1 == b->s1 && a->s2 == b->s2;
}
...
void simplcfg(Fn *fn) {
        ...
        do {
                ...
                for (b = fn->start; b; b = b->link) {
                        ...
                        } else if (j->type == Jjnz && empty[j->s1->id] && empty[j->s2->id] && jmpeq(&jmp[j->s1->id], &jmp[j->s2->id]) && jmpnophi(&jmp[j->s1->id])) {
                                *j   = jmp[j->s1->id];
                                done = 0;
                        }
                }
