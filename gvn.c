...
assoccon(Fn *fn, Blk *b, Ins *i1) {
        ...
        if (!optab[op].assoc || KBASE(i1->cls) != 0 || rtype(i1->arg[0]) != RTmp || rtype(i1->arg[1]) != RCon) {
                return;
        }
        ...
        if (op == Oadd && c.type == CBits) {
                if ((i1->cls == Kl && c.bits.i < 0) || (i1->cls == Kw && (int32_t)c.bits.i < 0)) {
...
branchdom(Fn *fn, Blk *bif, Blk *bbr1, Blk *bbr2, Blk *b) {
        ...
        if (b != bif && dom(bbr1, b) && !reachesnotvia(fn, bbr2, b, bif)) {
                return 1;
        }
...
int zeroval(Fn *fn, Blk *b, Ref r, int cls, int *z) {
        ...
        for (d = b->idom; d; d = d->idom) {
                ...
                if (cmpeqz(fn, d->jmp.arg, &arg, &cls1, &eqval) && req(r, arg) && cls == cls1 && domzero(fn, d, b, z)) {
                        *z ^= eqval;
                        return 1;
                }
        }
...
dedupjmp(Fn *fn, Blk *b) {
        ...
        v = 1;
        z = 0;
        if (b->s1 == b->s2 || isconbits(fn, b->jmp.arg, &v) || zeroval(fn, b, b->jmp.arg, Kw, &z)) {
                if (v == 0 || z) {
                        b->s1 = b->s2;
                }
