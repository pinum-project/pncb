...
fixarg(Ref *r, int k, Ins *i, Fn *fn) {
        ...
        if (KBASE(k) == 1 && rtype(r0) == RCon) {
                ...
                fn->mem[fn->nmem - 1] = a;
        } else if (op == Ocall && r == &i->arg[0] && rtype(r0) == RCon && fn->con[r0.val].type != CAddr) {
        ...
        } else if (op != Ocall && hascon(r0, &c, fn) && c->type == CAddr && ((c->sym.type & SExt) || (T.apple && c->sym.type == SThr))) {
                r1 = newtmp("isel", Kl, fn);
                if (c->bits.i) {
        ...
        } else if (!(isstore(op) && r == &i->arg[1]) && !isload(op) && op != Ocall && rtype(r0) == RCon && fn->con[r0.val].type == CAddr) {
...
selsel(Fn *fn, Blk *b, Ins *i, Num *tn) {
        ...
        } else if (fi->op == Oand && t->nuse == 1 && (rtype(fi->arg[0]) == RTmp || rtype(fi->arg[1]) == RTmp)) {
                fi->op = Oxtest;
                fi->to = R;
...
seljmp(Blk *b, Fn *fn) {
        ...
        if (!fi || !req(fi->to, r)) {
                ...
                b->jmp.type = Jjf + Cine;
        } else if (iscmp(fi->op, &k, &c) && c != NCmpI + Cfeq /* see sel(), selsel() */
                   && c != NCmpI + Cfne) {
                swap = cmpswap(fi->arg, c);
                if (swap) {
                        c = cmpop(c);
...
static uchar *matcher[] = {
    [Pbi1] = (uchar[]){
        1, 3, 1, 3, 2, 0},
    [Pbis]  = (uchar[]){5, 1, 8, 5, 27, 1, 5, 1, 2, 5, 13, 3, 1, 1, 3, 3, 3, 2, 0, 1, 3, 3, 3, 2, 3, 1, 0, 1, 29},
    [Pob]   = (uchar[]){1, 3, 0, 3, 1, 0},
    [Pobi1] = (uchar[]){5, 3, 9, 9, 10, 33, 12, 35, 45, 1, 5, 3, 11, 9, 7, 9, 4, 9, 17, 1, 3, 0, 3, 1, 3, 2, 0, 3, 1, 1, 3, 0, 34, 1, 37, 1, 5, 2, 5, 7, 2, 7, 8, 37, 29, 1, 3, 0, 1, 32},
    [Pobis] = (uchar[]){5, 2, 10, 7, 11, 19, 49, 1, 1, 3, 3, 3, 2, 1, 3, 0, 3, 1, 0, 1, 3, 0, 5, 1, 8, 5, 25, 1, 5, 1, 2, 5, 13, 3, 1, 1, 3, 3, 3, 2, 0, 1, 3, 3, 3, 2, 26, 1, 51, 1, 5, 1, 6, 5, 9, 1, 3, 0, 51, 3, 1, 1, 3, 0, 45},
        ...
        *a = (Addr){co, rb, ri, s};
        ...
        if (rtype(ri) == RTmp) {
                if (fn->tmp[ri.val].slot != -1) {
                        if (a->scale != 1 || fn->tmp[rb.val].slot != -1) {
                                return 0;
                        }
                        a->base  = ri;
                        a->index = rb;
